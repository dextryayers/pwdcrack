// ============================================================
// AES-CMAC Core
// AES-128 encryption + subkey generation (K1, K2)
// Implements NIST SP 800-38B
// ============================================================

module aes_cmac_core (
    input  logic        clk,
    input  logic        rst_n,
    input  logic        start,
    input  logic [127:0] key,
    input  logic [127:0] message,
    output logic [127:0] mac,
    output logic         ready
);

    typedef enum logic [2:0] { IDLE, SUBKEY, ENCRYPT, CMAC_XOR, DONE } state_t;
    state_t state;

    logic [127:0] K1, K2;
    logic [127:0] last_block;
    logic [127:0] enc_out;
    logic [127:0] aes_key;
    logic [127:0] aes_data_in;
    logic         aes_enc_start, aes_enc_done;
    logic [127:0] aes_cipher;

    // AES-128 Encrypt (simplified SubBytes, ShiftRows, MixColumns, AddRoundKey)
    logic [7:0] sbox [255];
    logic [127:0] round_key [10];
    logic [3:0] rcon;
    ...
    always_comb begin
        ...
        for (int r = 1; r < 11; r++) begin
            logic [31:0] temp = round_key[r-1][127:96] ^
                                (SubWord(RotWord(round_key[r-1][31:0])) ^ {24'h0, 8'(2**(r-1))});
            round_key[r][127:96] = round_key[r-1][95:64] ^ temp;
            round_key[r][95:64]  = round_key[r-1][63:32] ^ round_key[r][127:96];
            round_key[r][63:32]  = round_key[r-1][31:0]  ^ round_key[r][95:64];
            round_key[r][31:0]   = temp ^ round_key[r][127:96] ^ round_key[r][95:64] ^ round_key[r][63:32];
        end
    end

    // AES encrypt round
    function automatic logic [127:0] aes_round(input logic [127:0] state_in, input logic [127:0] rk);
        logic [127:0] sb, sr, mc;
        // SubBytes
        for (int i = 0; i < 16; i++)
            sb[i*8+:8] = sbox[state_in[i*8+:8]];
        // ShiftRows
        sr[127:96] = sb[127:96];
        sr[95:64]  = {sb[71:64], sb[95:72]};
        sr[63:32]  = {sb[47:32], sb[63:48]};
        sr[31:0]   = {sb[23:16], sb[31:24]};
        // MixColumns (simplified — GF mult for AES)
        for (int c = 0; c < 4; c++) begin
            logic [7:0] a0, a1, a2, a3;
            a0 = sr[(8*4*c)+:8]; a1 = sr[(8*4*c+8)+:8];
            a2 = sr[(8*4*c+16)+:8]; a3 = sr[(8*4*c+24)+:8];
            mc[(8*4*c)+:8]   = gf_mul2(a0) ^ gf_mul3(a1) ^ a2 ^ a3;
            mc[(8*4*c+8)+:8]  = a0 ^ gf_mul2(a1) ^ gf_mul3(a2) ^ a3;
            mc[(8*4*c+16)+:8] = a0 ^ a1 ^ gf_mul2(a2) ^ gf_mul3(a3);
            mc[(8*4*c+24)+:8] = gf_mul3(a0) ^ a1 ^ a2 ^ gf_mul2(a3);
        end
        return mc ^ rk;
    endfunction
    ...
    function automatic logic [127:0] aes_enc(input logic [127:0] pt, input logic [127:0] rk0);
        logic [127:0] st = pt ^ rk0;
        for (int r = 1; r <= 10; r++) begin
            if (r == 10)
                st = aes_round_last(st, round_key[r]);
            else
                st = aes_round(st, round_key[r]);
        end
        return st;
    endfunction

    function automatic logic [127:0] aes_round_last(input logic [127:0] state_in, input logic [127:0] rk);
        logic [127:0] sb, sr;
        for (int i = 0; i < 16; i++)
            sb[i*8+:8] = sbox[state_in[i*8+:8]];
        sr[127:96] = sb[127:96];
    ...
    // Subkey generation (NIST SP 800-38B)
    function automatic logic [127:0] dbl(input logic [127:0] x);
        logic msb = x[127];
        logic [127:0] shifted = {x[126:0], 1'b0};
        return msb ? (shifted ^ 128'h87) : shifted;
    endfunction
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            ENCRYPT: begin
                aes_data_in <= key;
                aes_cipher <= aes_enc(aes_data_in, round_key[0]);
                state <= SUBKEY;
            end
            SUBKEY: begin
                K1 <= dbl(aes_cipher);
                K2 <= dbl(dbl(aes_cipher));
                last_block <= message;
                state <= CMAC_XOR;
            end
            CMAC_XOR: begin
                aes_data_in <= last_block ^ K1;
                aes_cipher <= aes_enc(aes_data_in, round_key[0]);
                state <= DONE;
            end
            DONE: state <= IDLE;
