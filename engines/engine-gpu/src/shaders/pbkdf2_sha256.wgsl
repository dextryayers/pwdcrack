// PBKDF2-HMAC-SHA256 compute shader
...
fn sha256_hash(data: array<u32, 16>, len: u32) -> array<u32, 8> {
...
fn hmac_sha256(msg: array<u32, 16>, msg_len: u32, key: array<u32, 16>, klen: u32) -> array<u32, 8> {
    var ipad: array<u32, 16>;
    var opad: array<u32, 16>;
    for (var i: u32 = 0u; i < 16u; i++) {
        ipad[i] = key[i] ^ 0x36363636u;
        opad[i] = key[i] ^ 0x5c5c5c5cu;
    }
    var inner: array<u32, 16>;
    for (var i: u32 = 0u; i < 16u; i++) { inner[i] = ipad[i]; }
    for (var i: u32 = 0u; i < msg_len; i++) {
        let w = i / 4u;
        let b = i % 4u;
        if (w < 16u && w < 16u) {
            let m = 0xffu << (b * 8u);
            inner[w] = (inner[w] & ~m) | (msg[w] & m);
        }
    }
    let inner_hash = sha256_hash(inner, klen + msg_len);
    var outer: array<u32, 16>;
    for (var i: u32 = 0u; i < 16u; i++) { outer[i] = opad[i]; }
    for (var i: u32 = 0u; i < 8u; i++) { outer[4u + i] = inner_hash[i]; }
    return sha256_hash(outer, klen + 32u);
}

fn xor_block(a: array<u32, 8>, b: array<u32, 8>) -> array<u32, 8> {
    var out: array<u32, 8>;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var i: u32 = 0u; i < 16u; i++) { key[i] = 0x0u; }
    let salt_start = config.pcount * 16u;
    var salt_words = config.salt_len;
    if (salt_words > 16u) { salt_words = 16u; }
    for (var i: u32 = 0u; i < salt_words; i++) { key[i] = input[salt_start + i]; }

    var state: array<u32, 8>;
    for (var i: u32 = 0u; i < 8u; i++) { state[i] = 0u; }

    for (var iter: u32 = 0u; iter < config.iterations; iter++) {
        var h = hmac_sha256(msg, msg_len, key, salt_words);
        if (iter == 0u) {
            for (var i: u32 = 0u; i < 8u; i++) { state[i] = h[i]; }
        } else {
            state = xor_block(state, h);
        }
        for (var i: u32 = 0u; i < 8u; i++) { key[i] = h[i]; }
    }

    let out_base = idx * 8u;
    for (var i: u32 = 0u; i < 8u; i++) { output[out_base + i] = state[i]; }
}
