// Optimized MD5 — 64 steps fully unrolled (loop-based but compact)
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var i: u32 = 0u; i < len; i++) {
        let word_idx = i / 4u;
        let byte_idx = i % 4u;
        M[word_idx] |= u32(pw[i]) << (byte_idx * 8u);
    }
    let bit_len = len * 8u;
    let byte_off = len % 4u;
    ...
    let out_base = idx * 4u;
    output[out_base] = 0x67452301u + a;
    output[out_base + 1u] = 0xefcdab89u + b;
    output[out_base + 2u] = 0x98badcfeu + c;
    output[out_base + 3u] = 0x10325476u + d;
}
