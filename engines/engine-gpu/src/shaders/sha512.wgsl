// SHA-512 compute shader — 64 passwords per workgroup
...
fn sha512_compress(state: ptr<function, array<u64, 8>>, block: array<u64, 16>) {
  ...
  for (var i: u32 = 16u; i < 80u; i++) {
    let s0 = ((w[i-15] >> 1u) | (w[i-15] << 63u)) ^
             ((w[i-15] >> 8u) | (w[i-15] << 56u)) ^ (w[i-15] >> 7u);
    let s1 = ((w[i-2] >> 19u) | (w[i-2] << 45u)) ^
             ((w[i-2] >> 61u) | (w[i-2] << 3u)) ^ (w[i-2] >> 6u);
    w[i] = w[i-16] + s0 + w[i-7] + s1;
  }
  var a = (*state)[0]; var b = (*state)[1]; var c = (*state)[2]; var d = (*state)[3];
  var e = (*state)[4]; var f = (*state)[5]; var g = (*state)[6]; var h = (*state)[7];
  for (var i: u32 = 0u; i < 80u; i++) {
    let S1 = ((e >> 14u) | (e << 50u)) ^ ((e >> 18u) | (e << 46u)) ^ ((e >> 41u) | (e << 23u));
    let ch = (e & f) ^ ((~e) & g);
    let temp1 = h + S1 + ch + K64[i] + w[i];
    let S0 = ((a >> 28u) | (a << 36u)) ^ ((a >> 34u) | (a << 30u)) ^ ((a >> 39u) | (a << 25u));
    let maj = (a & b) ^ (a & c) ^ (b & c);
    let temp2 = S0 + maj;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var i: u32 = 0u; i < len; i++) {
    let byte_idx = i % 8u;
    let word_idx = i / 8u;
    w[word_idx] |= u64(password_bytes[i]) << (56u - byte_idx * 8u);
  }
  ...
  sha512_compress(&state, w);

  let matched = (state[0] == u64(target[0]) << 32u | u64(target[1]) &&
                 state[1] == u64(target[2]) << 32u | u64(target[3]) &&
                 state[2] == u64(target[4]) << 32u | u64(target[5]) &&
                 state[3] == u64(target[6]) << 32u | u64(target[7]));
