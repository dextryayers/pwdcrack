// RIPEMD-128 — 64 steps, 512-bit block
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    var h: array<u32, 4> = array(0x67452301u, 0xefcdab89u, 0x98badcfeu, 0x10325476u);
    var a = h[0]; var b = h[1]; var c = h[2]; var d = h[3];
    var a2 = h[0]; var b2 = h[1]; var c2 = h[2]; var d2 = h[3];
    ...
    for (var j: u32 = 0u; j < 64u; j++) {
        ...
        a = d; d = c; c = b; b = b + rol(t, S[j]);

        var fj2: u32;
        if (rnd == 0u) { fj2 = f4(b2, c2, d2); }
        else if (rnd == 1u) { fj2 = f3(b2, c2, d2); }
        else if (rnd == 2u) { fj2 = f2(b2, c2, d2); }
        else { fj2 = f1(b2, c2, d2); }
        let t2 = a2 + fj2 + w[R2[j] & 15u] + select(0x0u, 0x50a28be6u, rnd == 0u) +
                 select(0x5c4dd124u, 0x6d703ef3u, rnd == 2u);
        a2 = d2; d2 = c2; c2 = b2; b2 = b2 + rol(t2, S2[j]);
    }

    let tmp = h[0]; h[0] = h[0] + a + b2; h[1] = h[1] + b + c2;
    h[2] = h[2] + c + d2; h[3] = h[3] + d + a2;
